...
const countryMap = {
    ...
    BM: "Bermuda", BN: "Brunei Darussalam", BO: "Bolivia", BQ: "Bonaire, Sint Eustatius and Saba",
    ...
    ES: "Spain", ET: "Ethiopia", FI: "Finland", FJ: "Fiji", FK: "Falkland Islands (Malvinas)",
    ...
    PY: "Paraguay", QA: "Qatar", RE: "Réunion", RO: "Romania", RS: "Serbia", RU: "Russian Federation",
    ...
    TT: "Trinidad and Tobago", TV: "Tuvalu", TW: "Taiwan, Province of China", TZ: "Tanzania, United Republic of",
  ...
  function getCountryName(code: string) {
    return countryMap[code.toUpperCase() as keyof typeof countryMap] || "Unknown Country";
  }
  ...
  useEffect(() => {
    const fetchIP = async () => {
      try {
        ...
        if (countryLine) {
            const extractedCountry = countryLine.split('=')[1];
            setCountry(getCountryName(extractedCountry));
        } else {
            throw new Error('Country not found in response');
        }
        
        if (ipLine) {
          const extractedIP = ipLine.split('=')[1];
          setIp(extractedIP);
        }  else {
