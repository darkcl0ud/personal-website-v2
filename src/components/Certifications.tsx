        ...
        <Section>
            ...
            <ul className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,24rem),1fr))] mt-16 gap-8 items-center justify-center">
                {certifications.map((certification) => (
                    <li key={certification.code} className="group bg-neutral-500/60 p-8 outline outline-white/5 h-[16rem] flex flex-col gap-4 hover:cursor-pointer">
                        ...
                        <p className="text-tertiary-100 font-extralight tracking-widest text-xs">EXPIRES: {certification.expires}</p>
                    </li>
                ))}
            </ul>
        </Section>
